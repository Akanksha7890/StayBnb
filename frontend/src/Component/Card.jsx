...
function Card({ title, landMark, image1, image2, image3, rent, city, id, ratings, isBooked, host }) {
    ...
    return (
      <div 
    className='w-[330px] max-w-[85%] h-[460px] flex items-start justify-start flex-col rounded-lg cursor-pointer relative z-[10] transition-all duration-300 ease-in-out hover:shadow-2xl hover:scale-[1.02]' 
    onClick={() => !isBooked ? handleClick() : null}
>
            {/* Sirf tabhi dikhega agar isBooked true hai AND user login hai */}
            {isBooked && userData && (
            ...
            <div className=' w-[100%] h-[33%] py-[20px] flex flex-col gap-[2px]'>
                ...
                <span className='text-[16px] font-semibold text-[#fa4c4c]'>₹{rent}/day</span>
            </div>
